...
- steps / cfg
  ...
  - 現在の public API / CLI からは未露出
- denoise
  - 内部関数 `apply_overrides()` では `denoise` 入力を書き換える
  - public API / CLI から露出し、I2I では元画像の保持量調整に利用する。ComfyUI の denoise 値として扱うため、値が小さいほど元画像を保持しやすい
### 10.4 link 走査方針
...
`generate` / `generate-i2i` の `--output` は `GenerationResult.images` の先頭画像を bytes のまま指定パスへ保存する。I2I でも保存後に `GenerationResult` の JSON を標準出力へ表示する。
- `models`
  - positional `folder`

`--lora-yaml`, `--width`, `--height`, `--steps`, `--cfg`, `--timeout`, `--verbose` は当初案に含めていたが、現在の CLI では未実装。`--denoise` は実装済みとし、meta YAML の `denoise` binding がある workflow で使用できる。
